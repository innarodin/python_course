...
class Knot(Polyline):  
    ...
    def get_point(self, points, count):
        get_knot(points, count)
        
    def set_points(self, points, count):
        get_knot(points, count)
...
if __name__ == "__main__":
    ...
    while working:
        ...
        line = Polyline()
        line.draw_points(points)
        knot = Knot()
        knot.draw_points(knot.set_points(points, steps), "line", 3, color)
        if not pause:
            line.set_points(points, speeds)
        if show_help:
            draw_help()
